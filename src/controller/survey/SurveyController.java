...
public class SurveyController extends HttpServlet {
	...
	protected void doProc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (url.indexOf("index.do") != -1) {
            ...
            // menu_gubun 변수에 survey_index 을 할당
        ...
        } else if (url.indexOf("list.do") != -1 || url.indexOf("survey.do") != -1 ) {
            ...
            if (url.indexOf("list.do") != -1) {
                temp = "/survey/list.jsp";
            } else if (url.indexOf("survey.do") != -1) {
                temp = "/survey/survey.jsp";
            }
            RequestDispatcher rd = request.getRequestDispatcher(temp);
            rd.forward(request, response);
            ...
            if (result > 0) { // 성공
                System.out.println("-- 설문 답변 성공 --");
                return;
            } else { // 실패
                System.out.println("-- 설문 답변 실패 --");
                return;
            }
            ...
            if (result > 0) { // 성공
                System.out.println("-- 설문 수정 성공 --");
                return;
            } else { // 실패
                System.out.println("-- 설문 수정 실패 --");
                return;
            }
            
        } else if (url.indexOf("answerSetProc") != -1) {
            
            String survey_answer_set = request.getParameter("survey_answer_set");
            System.out.println("survey_answer_set : " + survey_answer_set);
            String[] survey_answer_setArr = survey_answer_set.split("[|]");
            System.out.println("survey_answer_setArr.length : " + survey_answer_setArr.length);
            
            for (int i=0; i<survey_answer_setArr.length; i++) {
                String[] imsiArr = survey_answer_setArr[i].split(":");
